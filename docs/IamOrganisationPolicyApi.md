# h1-client-python.IamOrganisationPolicyApi
...
with h1-client-python.ApiClient(configuration) as api_client:
    ...
    api_instance = h1-client-python.IamOrganisationPolicyApi(api_client)
    organisation_id = 'organisation_id_example' # str | Organisation Id
policy_id = 'policy_id_example' # str | Policy Id
iam_actor = h1-client-python.IamActor() # IamActor | 
...
> list[IamActor] iam_organisation_policy_actor_list(organisation_id, policy_id)
...
> Policy iam_organisation_policy_create(organisation_id, iam_project_policy_create, x_idempotency_key=x_idempotency_key)
...
with h1-client-python.ApiClient(configuration) as api_client:
    ...
    api_instance = h1-client-python.IamOrganisationPolicyApi(api_client)
    organisation_id = 'organisation_id_example' # str | Organisation Id
iam_project_policy_create = h1-client-python.IamProjectPolicyCreate() # IamProjectPolicyCreate | 
x_idempotency_key = 'x_idempotency_key_example' # str | Idempotency key (optional)

    try:
        # Create iam/policy
        api_response = api_instance.iam_organisation_policy_create(organisation_id, iam_project_policy_create, x_idempotency_key=x_idempotency_key)
        pprint(api_response)
    except ApiException as e:
...
> list[Event] iam_organisation_policy_event_list(organisation_id, policy_id, limit=limit, skip=skip)
...
> list[Policy] iam_organisation_policy_list(organisation_id, name=name, tag_value=tag_value, tag_key=tag_key)
...
tag_key = 'tag_key_example' # str | Filter by tag.key (optional)
    ...
    try:
        # List iam/policy
        api_response = api_instance.iam_organisation_policy_list(organisation_id, name=name, tag_value=tag_value, tag_key=tag_key)
        pprint(api_response)
    except ApiException as e:
...
> list[ResourceService] iam_organisation_policy_service_list(organisation_id, policy_id)
...
> list[Tag] iam_organisation_policy_tag_list(organisation_id, policy_id)
...
> list[Tag] iam_organisation_policy_tag_put(organisation_id, policy_id, tag)
...
tag = [h1-client-python.Tag()] # list[Tag] | 
    ...
    try:
        # Replace iam/policy.tag
        api_response = api_instance.iam_organisation_policy_tag_put(organisation_id, policy_id, tag)
        pprint(api_response)
    except ApiException as e:
...
with h1-client-python.ApiClient(configuration) as api_client:
    ...
    organisation_id = 'organisation_id_example' # str | Organisation Id
policy_id = 'policy_id_example' # str | Policy Id
iam_project_policy_update = h1-client-python.IamProjectPolicyUpdate() # IamProjectPolicyUpdate | 
